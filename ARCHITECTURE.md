...
| Module | Holds |
|---|---|
| `workflow.py` | The `@workflow` decorator, the `Run` object a workflow is handed, and `Stop`. A workflow is a decorated async function, never a subclass. **`version` and nothing else** — the decorator carries only what nothing else can see, and the three arguments that went each restated something the framework already had. `params=` was the annotation on the workflow function's own first parameter, a declaration mypy already enforces, so UF2.2 made `Workflow.params` read that annotation instead — lazily, when it is asked for, because a params class declared *below* its own workflow is not bound when the decorator runs — and refuse rather than default when it cannot: an unannotated `run` is not a bare `Run`. The pair could disagree for as long as there were two of them — `Run` is covariant, so `@workflow(params=FixParams)` over `async def fix(run: Run)` type-checked exactly — and there is now one place to say it. `name=` was the entry-point key and is now only that: the decorator held a copy that agreed with the key by convention, that nothing anywhere compared it against, and that a package registered under another key contradicted in silence. `roles=` was the `@role(model=…)` registry: the decorator binds `(name, model)` onto the factory object at import, and the module a workflow's `def` ran in is therefore already the registry preflight scans — a list on this line was the same thing said twice, and the copy an author kept by hand was the one free to be wrong. What is left is what nothing can infer: no annotation, namespace or key says *"I changed the shape of this workflow"*, and `version` is what stands between a resume and a record replayed into reordered steps. Also re-exports the three port names a `Run`'s members speak — `Namespace` (what `worktree()` refuses through), `Conflict` and `VerifierOutcome` (what `integrate()`'s outcome carries to a conflict view) — for the reason `roles.py` carries `Claude`; see §5 |
