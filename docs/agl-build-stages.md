...
**A checksum file at a path another agent also writes is not a checksum file.** Stage 18 had two
mutation passes share one scratchpad; a snapshot overwrote a subagent's manifest, after which
`shasum -c` silently checked nine files instead of 224 and reported all-clear — for a question it had
not asked. Give each pass its own path, and check the file count as well as the verdict.
...
| # | Deliverable |
|---|---|
| 11.0 | **Move composition out of `main.py`, before anything else.** Stage 10 found that `main.py` resolves the project and builds the container *before* dispatching, which makes `init` unreachable — it writes the very project file the container needs to be constructible — and makes `workflows` demand a registered repository to list what is merely installed. `main.py` resolves only settings, then dispatches; each `api` operation resolves its own prerequisites. `run`/`resume`/`clear` resolve a project and build a container; `init` takes settings alone; `list_workflows` takes neither. An unregistered repository is `NotFoundError` → exit 3, naming `agl init`, which is run once per project. `container.py` stays the only place that says `new`. ~20 lines relocated, one consumer, no new abstraction |
...
| # | Deliverable |
|---|---|
| 16.1 | Preflight — provider availability (**harness binary on `PATH`, version, authenticated session** — not merely a credential), capability match, `on_question` requires a provider that can ask (§3.2). Stage 15 flagged one collision to catch here if it can be caught at all: Codex's `tool_timeout_sec` is a vendor clock on the tool carrying a question, and §3.7 has no timeouts by design — on the backend with no second asking mechanism, a timeout ends the question with nothing to fall back to |
| 16.2 | `cli/commands/resume.py` — label only, params from `run.json`. **`api.resume` needs `async with services.terminal` exactly as `api.run` does, and nothing in the repository would notice its absence** — before 15.1 nothing entered a `Terminal` at all, so every `show` in every real run raised `InternalError`, invisible because no SDK test went through `api.run` |
| 16.3 | `cli/commands/clear.py` — `git branch -d` semantics, refuses while locked (§3.10) |
| 16.4 | `cli/commands/init.py` + `workflows.py`. **`init` needs a starting directory** — §3.10 asks it to take settings alone *and* detect the git root, but after 11.0 `Path.cwd()` lives inside the deferred thunk, so it either reads cwd ambiently inside `api.py` (below `cli/`) or grows a `cwd` parameter and is no longer settings alone. Composition order was repaired at 11.0, so both are reachable; `init` takes settings alone and `workflows` takes neither. Settle the filename while here: `ports/home_layout.py` has `config.toml` and §3.10 now agrees. And decide the UX gap stage 10 declined: `agl run fix -h` cannot show that workflow's own flags, and a colliding workflow flag cannot be refused, without loading the workflow to inspect it |
| 16.5 | Two carried findings first. **`FakeServices` holds `terminal` twice** — inside `services` and as a sibling field — so after `dataclasses.replace(services, terminal=…)` the two silently disagree and a test reading both reads the wrong object. And **`from agl.sdk import Screen` is false as built**: `ARCHITECTURE.md` §5 and `ports/terminal.py` both spell it that way, but the facades expose `agl.sdk.terminal` / `agl.sdk.questions`. Either reword both, or give `sdk/__init__.py` a deliberate full re-export — a decision about the SDK's front door, and it bears on stage 17 where R1 counts lines. **Split across two modules — stage 9 found `sdk/testing.py` unbuildable as one.** Contract 1 puts `sdk` below `config`, so it cannot import the container and cannot build a bundle at all; building one is a composition act. The scripting *vocabulary* stays in `sdk/testing.py`, port-typed. The *builder* becomes a new top-level `agl/testing.py` beside `api.py`, above `config`. Note also the constraint stage 7 found: `sdk/` and `adapters/` are siblings and may not import each other, so the vocabulary module **cannot name the fake's scripting types**. The workflow-facing scripting vocabulary lives here and `config/container.py` compiles it into the callable the fake consumes. The harness workflow authors test against: run a workflow on an all-fakes bundle, script agent replies and questions, drive kill-and-resume. Part of the public SDK surface, so it needs the same care as the rest of it |
...
| # | Deliverable |
|---|---|
| 17.0 | **Two real defects from stage 16, first.** (i) `clear` keeping an unmerged `agl/<label>` means a later `agl run … -n <label> --from <ref>` **silently ignores `--from`** and starts from the old tip, because `open` takes its attaching path and `base` is consulted only when provisioning. (ii) `clear` is specified to refuse while a run holds a lock and v1.1 has no mechanism — a `flock` on the run directory, held for the life of the process, is the shape (§3.9 already uses it for the registry; it is an OS lock, not the stored status §3.11 refuses). Also: `api.py`'s "one `agl clear` away" is off by a flag — a run that crashed between `write_record` and `open` names a branch that was never created, so `contains` raises `NotFoundError` and `-f` is what clears it |
| 17.1 | `workflows/fix/` — params, roles (**Claude implements, OpenAI reviews**), prompts with build commands written in literally |
| 17.2 | `workflows/fix/views/` — one board, one question screen. **Stage 16 flagged a blocker here: AGL ships no input-capable `Terminal` an external author can drive.** Answering a screen on fakes needs `RichTerminal` plus a `Keys` implementation the author writes. Either the harness grows one or this stage writes it — but a workflow author hitting this is an R1 failure, so decide it as a surface question rather than a test fixture |
...
| # | Deliverable |
|---|---|
| 18.0 | **The harness gaps stage 17 measured — R1 failures, not fixtures.** (i) **`agl/testing.py` must ship an input-capable `Terminal`**: wiring §3.7's question screen turns a default harness run of `fix` from green to **exit 6**, and answering one today costs the author the `agl[terminal]` extra, a hand-written `Keys` implementation, and an import of `agl.adapters.rich_terminal.terminal` — a module contract 6 forbids their workflow to touch. Shape: `testing.answering([...])` handed to `harness(terminal=…)`, queueing interactive screens by priority so `pending` stays honest and keeping the slot so a test can read the board back. It must pass `tests/contracts/terminal.py` as a third class — that is the price, and why it is a surface change rather than a fixture. (ii) **A `Run` factory on `agl.testing`, with `activity=`.** A §3.7-shaped board takes the live `Run`, so testing one means building eight fake ports — and `run.activity` is settable from nowhere public, which cost stage 17 the worst line in the stage (`run._steps._activity`, a private `_engine` attribute written from a workflow's own test). (iii) **`agl.sdk` re-exports no exception class**, so an author asserting how their run refused has nowhere to import from and reaches into `agl.ports.errors`. (iv) Document that **a scripted agent returning only a `Reply` commits nothing** — `commit_all` is a no-op on an unmoved tree, so the harness's own example agent makes all three `commit=` arguments indistinguishable. The single biggest trap for an author testing a `commit=`, mentioned nowhere |
