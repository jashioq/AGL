...
This file is the other half of `qualified.py` and has no independent claim. The shipped shape puts
a workflow's roles in a `roles.py` beside it - `workflows/fix/roles.py` is the instance - and an
author then reaches them by one of two imports: `from .roles import implementer`, which binds a
factory in the workflow's namespace, or `from . import roles`, which binds this module and nothing
else. The second is the spelling `qualified.py` measures, and it needs a module to bind.
