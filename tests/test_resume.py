...
async def test_a_file_edited_in_the_workflow_directory_refuses_the_resume_and_is_named(
    ...
    **The file is named and no other file is.** A refusal saying only "the workflow changed" ends
    ...
    assert (
        "`agl clear auth`, which deletes the run, its worktrees and branches, and any uncommitted "
        "work in them" in message
    )
    assert "Then `agl run triage -n auth` with the same flags as before starts" in message
